...
// stb declarations only — the implementation bodies are compiled exactly
// once, in src/stb_impl.cpp. See docs/coding_guidelines.md Sec 10.
...
// clang-format off
vc::vc_image stb_image_reader::read(const path& p,
    ...
    //      Always pass desired_channels = 0 — never force a conversion (Sec 7.4).
...
void stb_image_writer::write(const path& p,
    ...
    // clang-format on
    (void)image;
    (void)config; // TODO(you): dispatch on config.format once more than PNG is supported
    ...
    throw vc::vc_exception(vc::vc_error_code::encode_error,
                           "stb_image_writer::write: not yet implemented (" + p + ")");
}
