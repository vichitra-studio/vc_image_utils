...
static_assert(
    vc_optional_reader_req<vc_cached_edits_table, std::string, data_bytes>);
...
static_assert(
    vc_optional_reader_req<vc_persistent_edits_table, std::string, data_bytes>);
static_assert(
    vc_writer_req<vc_persistent_edits_table, std::string, data_bytes>);

vc_cached_edits_table::vc_cached_edits_table(i_table& backing)
    : backing_(backing) {
}

std::optional<data_bytes>
vc_cached_edits_table::get(const std::string& key) const {
...
std::optional<data_bytes>
vc_persistent_edits_table::get(const std::string& key) const {
    // TODO(you): return backing_.get(key) — a hit yields the bytes, a miss
